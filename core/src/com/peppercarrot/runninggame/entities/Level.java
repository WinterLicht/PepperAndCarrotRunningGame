...
/**
 * WIP... Level is constructed of TMX-parts. At the moment there are two
 * TMX-parts loaded. This level parts occur alterating in the level. Scroll
 * speed is increasind every few level parts that where loaded. TODO: more level
 * part to load, load game entities from TMX ...
 * 
 * @author WinterLicht
 *
 */
public class Level {
	int activeMap = 1;
	/** Helper, indicates which level should be loaded next. */
	public float scrollSpeed = 8;
	/** Horizontal scroll speed in pixel of the level. */
	int counter4SpeedIncr = 0;
	/** Counts level resets. */
	int speedIncrAt = 4;
	/** How often speed will be increased. */
	public boolean beginLevel = false;
	/** Set to true when player is ready. */
	...
	Stage enemies1;
	/** enemies of map part 1. */
	Stage enemies2;
	/** enemies of map part 2. */
	...
	/**
	 * Render background layers. It is assumed, that 0 and 1 are background
	...
	/**
	 * Render foreground layers. It is assumed, that 2 is foreground
	 */
		...
		for (int i = 1; i <= mapsAmount; i++) {
			if ((i - 1) * (1f / mapsAmount) <= random && random < i * (1f / mapsAmount)) {
				...
				if (counter4SpeedIncr >= speedIncrAt) {
					counter4SpeedIncr = 0;
					Gdx.app.log(getClass().getSimpleName(), "Speed increased");
					scrollSpeed += (1);
				}
				Gdx.app.log(getClass().getSimpleName(), "level" + i + ".tmx loaded");
			}
		}
	}
	 ...
	 * 
	 * @return array with integer y-values of all platform tops near the player
	 *         in pixel.
	 */
	public Array<Integer> getWallsYPosNearPlayer() {
		...
		if (activeMap == 1) {
			...
			float pos = (camera1.position.x - Constants.VIRTUAL_WIDTH / 2 + Constants.OFFSET_TO_EDGE)
					/ tiledMap1.getProperties().get("tilewidth", Integer.class);
			// Round this coordinates to get two near columns
			int min = (int) Math.floor(pos);
			int max = (int) Math.ceil(pos);
			if (min == max)
				max += 1; // Take then the one further column
			...
			float pos = (camera2.position.x - Constants.VIRTUAL_WIDTH / 2 + Constants.OFFSET_TO_EDGE)
					/ tiledMap2.getProperties().get("tilewidth", Integer.class);
			int min = (int) Math.floor(pos);
			int max = (int) Math.ceil(pos);
			if (min == max)
				max += 1;
		...
		if (i == 1) {
			...
			if (layer != null) {
				for (int column = 0; column < layer.getWidth(); column++) {
					for (int row = 0; row < layer.getHeight(); row++) {
						Cell cell = layer.getCell(column, row);
						if (cell != null) { // TODO: recognize which enemy by
											// cell property
							...
							// Not to forget the offset of camera center
							// +width/2
			...
			if (layer != null) {
				for (int column = 0; column < layer.getWidth(); column++) {
					for (int row = 0; row < layer.getHeight(); row++) {
						Cell cell = layer.getCell(column, row);
						if (cell != null) { // TODO: recognize which potion by
											// cell property
							int tileWidth = tiledMap1.getProperties().get("tilewidth", Integer.class);
							int tileHeight = tiledMap1.getProperties().get("tileheight", Integer.class);
							float posX = (column + 0.5f) * tileWidth;
							...
							potion.setX(
									posX - camera1.position.x + Constants.VIRTUAL_WIDTH / 2 - potion.getWidth() / 2);
				...
				for (int column = 0; column < layer.getWidth(); column++) {
					for (int row = 0; row < layer.getHeight(); row++) {
						...
						if (cell != null) {
							...
							potion.setOrigin(Align.center);
							potion.setX(
									posX - camera2.position.x + Constants.VIRTUAL_WIDTH / 2 - potion.getWidth() / 2);
							potion.setY(posY + Constants.OFFSET_TO_EDGE - potion.getHeight() / 2);
